...
class Parser(object):
    ...
    def feed(self, url, options):
        ...
        # Get salt
        if self.salt == None:
            salt_url = 'https://raw.githubusercontent.com/coslyk/moonplayer-plugins/master/iqiyi_salt.json'
            moonplayer.get_url(salt_url, self.get_salt_cb, options)
        else:
            moonplayer.get_url(url, self.parse_vid, options)
            
    def get_salt_cb(self, page, options):
        self.salt = str(json.loads(page)['salt'])
        moonplayer.get_url(self.origin_url, self.parse_vid, options)
    ...
    def parse_vms(self, content, options):
        ...
        if info['code'] != 'A000000':
            moonplayer.warn('[iqiyi] Error: Key is out-of-date.')
            return
        try:
            vs = info["data"]["vp"]["tkl"][0]["vs"]
        except:
            moonplayer.warn('[iqiyi] Error: Do not support vip video!')
        bids = [None] * 5
        for v in vs:
            bid = int(v[u'bid']) - 1
            if bid < 5 and bid >= 0:
